...
useSeoMeta({
    title: 'Wisdom Artisan - แก้ไขผลงาน',
});
...
const modal = ref({
    isOpen: false,
    type: 'success' as 'success' | 'error' | 'warning', // icon types
    title: '',
    message: '',
    onConfirm: null as (() => void) | null
});

// Helper: เปิด Modal แทน Alert
...
// Helper: ปิด Modal และทำงานตาม Callback (ถ้ามี)
const closeModal = () => {
...
onMounted(async () => {
    try {
        ...
        // 1. โหลดรายชื่อปราชญ์
        const artisanRes = await axios.get(`${baseUrl}/artisan`, { withCredentials: true });
        artisans.value = artisanRes.data;

        // 2. โหลดข้อมูลผลงานเดิม
        const galleryRes = await axios.get(`${baseUrl}/gallery/${galleryId}`, { withCredentials: true });
        const data = galleryRes.data;

        // 3. ใส่ข้อมูลใน Form
        form.value.name_gallery = data.name_gallery;
        form.value.caption = data.caption;
        form.value.artisan_id = data.artisan_id;

        // 4. จัดการ Preview รูปภาพ
        if (data.image_url) {
            imagePreview.value = data.image_url.startsWith('http')
                ? data.image_url
                : `${baseUrl}/uploads/gallery/${data.image_url}`;
        }
        ...
        showModal(
            "ข้อผิดพลาด", 
            "ไม่สามารถโหลดข้อมูลผลงานได้ หรือไม่พบข้อมูล", 
            "error", 
...
const submitForm = async () => {
    if (!form.value.name_gallery.trim()) {
        showModal("แจ้งเตือน", "กรุณากรอกชื่อผลงาน", "warning");
        return;
    }
    ...
    try {
        ...
        const targetUrl = `${baseUrl}/gallery/${galleryId}`;

        await axios.put(targetUrl, formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
            withCredentials: true
        });

        // สำเร็จ -> เปิด Modal -> พอกด OK ค่อย redirect
        showModal(
            "สำเร็จ", 
        ...
        showModal("เกิดข้อผิดพลาด", serverMsg || "ไม่สามารถบันทึกข้อมูลได้", "error");
    } finally {
...
<template>
    <div class="font-sans text-slate-800 p-6 min-h-screen relative">
        <div class="flex items-center gap-2 text-sm text-slate-500 mb-6">
            <NuxtLink to="/admin" class="hover:text-green-600 transition">Admin Dashboard</NuxtLink>
            <span>/</span>
            <NuxtLink to="/admin/gallery" class="hover:text-green-600 transition">จัดการแกลเลอรี่ผลงาน</NuxtLink>
            <span>/</span>
            <span class="font-medium text-slate-700">แก้ไขผลงาน</span>
        </div>

        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">แก้ไขข้อมูลผลงาน</h1>
                <p class="text-slate-500 text-sm mt-1">แก้ไขรายละเอียดหรือเปลี่ยนรูปภาพผลงาน</p>
            </div>
        </div>

        <div v-if="isLoading" class="flex justify-center py-20">
            <div class="w-10 h-10 border-4 border-green-200 border-t-[#428a4a] rounded-full animate-spin"></div>
        </div>

        <div v-else class="bg-white rounded-xl border border-gray-200 shadow-sm max-w-4xl mx-auto">
            <div class="p-6 md:p-8 space-y-8">

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-3">รูปภาพผลงาน</label>
                        ...
                        <label v-if="!imagePreview" for="image-upload"
                            ...
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                <div class="p-3 bg-green-100 rounded-full mb-3 group-hover:scale-110 transition duration-300">
                                    <svg class="w-8 h-8 text-[#428a4a]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2" />
                                    </svg>
                                </div>
                                <p class="mb-2 text-sm text-gray-500"><span class="font-semibold text-[#428a4a]">คลิกเพื่อเปลี่ยนรูป</span> หรือลากไฟล์มาวาง</p>
                                <p class="text-xs text-gray-400">หากไม่เลือกจะใช้รูปเดิม</p>
                            </div>
                    ...
                    <p class="text-xs text-gray-400 mt-2 text-center">* หากไม่ต้องการเปลี่ยนรูป ให้เว้นว่างไว้</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="col-span-2 md:col-span-1">
                        ...
                        <input v-model="form.name_gallery" type="text" placeholder="ชื่อผลงาน"
                            class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#428a4a] focus:border-transparent transition text-slate-700">
                    </div>

                    <div class="col-span-2 md:col-span-1">
                        <label class="block text-sm font-medium text-slate-700 mb-1">เจ้าของผลงาน (ปราชญ์)</label>
                        <div class="relative">
                            <select v-model="form.artisan_id" disabled
                                class="w-full px-4 py-2 bg-gray-100 border border-gray-200 rounded-lg text-slate-500 appearance-none cursor-not-allowed">
                        ...
                        <p class="text-xs text-orange-400 mt-1">* ไม่สามารถเปลี่ยนเจ้าของผลงานได้</p>
                    </div>

                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-slate-700 mb-1">คำอธิบายเพิ่มเติม</label>
                        <textarea v-model="form.caption" rows="4" placeholder="รายละเอียด..."
                            class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#428a4a] focus:border-transparent transition text-slate-700 resize-none"></textarea>
                    </div>
                </div>
